...
# OUTPUT should be a dict and the value only a float with the metric value
# Currently for knn and lin it is an array of len()=1 with a float, but this cannot be fixed bc of the way knn_acc works
...
def KNNEval(
    ...
    **kwargs,
    ...
    if test_dataset is None:
        ...
        # encode dataset
        embedding_cls, embedding_sep, embedding_av = wrapped_model.encode_dataset(
            data=dataset, device=device
        )
    ...
    # TRAIN/TEST SPLIT IN TRAINING
        ...
        # ENH: this code is inneficient since I have twice the same embeddings in memory.
        # encode train set
        embedding_cls_train, embedding_sep_train, embedding_av_train = (
            wrapped_model.encode_dataset(data=dataset, device=device)
        )
        # encode test set
        embedding_cls_test, embedding_sep_test, embedding_av_test = (
            wrapped_model.encode_dataset(data=test_dataset, device=device)
        )

        embedding_rep_dict = {
            "cls": (embedding_cls_train, embedding_cls_test),
